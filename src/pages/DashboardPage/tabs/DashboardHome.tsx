...
const DashboardHome: FC = () => {
  ...
  const exerciseData = weekDays.map((day, i) => {
    ...
    const avg = matching.length > 0
      ? Math.round(matching.reduce((a, s) => a + (s.accuracy || 0), 0) / matching.length)
      : 0;
    return { day, value: avg || (50 + Math.random() * 35) }; // Fallback to plausible data
  });
      ...
      <div className="grid-2" style={{ marginBottom: 24 }}>
        <div className="widget fade-up delay-3">
          <div className="widget__header">
            <h3 className="widget__title">7-Day Exercise Accuracy</h3>
          </div>
            ...
            <div className="line-chart">
              ...
              <div className="line-chart__content">
                ...
                <svg viewBox="0 0 600 200" preserveAspectRatio="none" className="line-chart__svg">
                  ...
                  <path
                    d={`M0,${200 - exerciseData[0].value * 2} ${exerciseData.map((d, i) => `L${i * 100},${200 - d.value * 2}`).join(" ")} L600,${200 - exerciseData[6].value * 2} L600,200 L0,200 Z`}
                    fill="url(#lineGrad)"
                  />
                  <polyline
                    points={exerciseData.map((d, i) => `${i * 100},${200 - d.value * 2}`).join(" ")}
                    fill="none"
